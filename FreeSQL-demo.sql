...
-- Descreve a estrutura dos tipos
DESC T_ARTISTA
DESC T_ALBUM
DESC T_FAIXA
...
-- CORREÇÃO: Uso de ALIAS 'f' obrigatório para acessar atributos do objeto no WHERE
DELETE FROM TB_FAIXA f WHERE f.faixa_obj.num_faixa > 5;
...
-- CORREÇÃO: Uso de ALIAS 'a' obrigatório no SET e WHERE para objetos
UPDATE TB_ALBUM a SET a.album_obj.gravadora = 'Universal Music'
WHERE a.id_album = 1;

-- CORREÇÃO: Uso de ALIAS 'a'
UPDATE TB_ALBUM a
SET a.album_obj.titulo = 'Nó na Orelha (Edição Especial)',
    a.album_obj.ano_lancamento = 2013
WHERE a.id_album = 1;

-- CORREÇÃO: Uso de ALIAS 'f'
UPDATE TB_FAIXA f
SET f.faixa_obj.duracao_seg = f.faixa_obj.duracao_seg + 10
WHERE f.faixa_obj.num_faixa = 1;
...
SELECT a.id_album, a.album_obj.get_descricao()
FROM TB_ALBUM a;
...
-- CORREÇÃO: Garantindo alias no DELETE
